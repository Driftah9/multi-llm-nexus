...
- **0.9.0** — convergence port of provider-neutral mechanisms (error classifier, memory injector, schema gate, council failover, identity, local-first research). See `docs/convergence-2026-06.md`.
- **Convergence hardening (PW-3→PW-8, Unreleased)** — boot-time config schema validation (`core/config_schema`); installer resilience (`scripts/preflight_check.sh`, `scripts/install_state.py`, `docs/INSTALLER_RESILIENCE.md` — local-only testing, no CI); `triage_validator` extensions (decision metadata, turn facts, re-ask detection, apex tier, legacy-DB migration); retired-model swap (Cerebras→`gpt-oss-120b`, SambaNova→`DeepSeek-V3.2`); `config/sessions.json` untracked. See `CHANGELOG.md` → Unreleased.
