...
config/
  ...
  spaces.yml             Operator-defined organizational spaces (optional)
...
Most consumer subscriptions (ChatGPT Plus, Gemini Advanced, X Premium, Copilot) are web UI subscriptions — they do not provide programmatic API access. The exception is **Claude Code** (subscription drives the CLI) and **GitHub Models** (GitHub/Copilot account gives API access to frontier models). All other providers require a separate API key with per-token billing or credits.
